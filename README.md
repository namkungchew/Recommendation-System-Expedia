...
In conclusion, we are satifised with the performance of our top model, the CategoricalNB as it perform well and achieve 22% about the baseline score. We are also excited about the potential on our TFRS model which achieved a kaggle score of 0.2194 and the other flexibility and configurations that we can do with it.
...
| Trainset MAPK Score | 0.3188                                   | 0.7354                        | 0.7224                     | 0.5600                   | 0.5554               | 0.9676        |
| Holdset MAPK Score  | 0.3188                                   | 0.6036                        | 0.5913                     | 0.4415                   | 0.3213               | 0.5695        |

We tested various different models using a small dataset which we filter out 10 classes and 100,000 data points from the main dataset. Above are the results. As expected, since our features are nominal features with no linearity within the features, K-Nearest Neighbor is not working well. We also tried Logistic Regression briefly which we didn't put in this notebook and that didn't work well either. Random Forest Classifier with OneHotEncoding will transform the original 14 features that we have to over 25,000 features which bogged the model and made the model needed a certain degree of depth to be effective. With TargetEncoding, it perform better than K Nearest Neighbour but still lagged behind CategoricalNB and TensorFlow Recommendation System
